...
<template>
    <div class="dialog">
        ...
        <!-- dialog组件 -->
        <el-dialog ref="dialogs" title="支付方式列表" append-to-body :visible.sync="show" :close-on-click-modal="false" width="70%">
            <el-row :gutter="10">
                <el-col :span="9">
                    <el-form-item label="支付方式编号" prop="paytypeid">
                        <el-input v-model="searchform.paytypeid" placeholder="请输入支付方式编号" @input="fetchTableData"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="9">
                    <el-form-item label="支付方式名称" prop="paytypename">
                        <el-input v-model="searchform.paytypename" placeholder="请输入支付方式名称" @input="fetchTableData"></el-input>
                    </el-form-item>
                </el-col>
            </el-row>
...
export default {
    ...
    methods: {
        ...
        showdiolog() {
            if (!this.disable) {
                this.searchform.paytypeid = '';
                this.searchform.paytypename = '';
                this.show = true;
                this.fetchTableData();
            }
        },
        ...
        // 回车事件
        inputEnterEvent() {
            this.searchform.paytypeid = this.str;
            this.$api.gpaytype
                .getDataByPage(
                    this.commEntity.pagination.pageIndex,
                    this.commEntity.pagination.pageSize,
